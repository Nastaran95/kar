...
if(isset($_GET) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
    ...
    if ($query==="=====+++=====") {
        if ($dastebandi=='all'){
            if ($type == 1)
                $results = $connection->query("SELECT COUNT(*) FROM karjoo_request");
        }else{
            if ($type == 1)
                $results = $connection->query("SELECT COUNT(*) FROM karjoo_request WHERE dastebandi='$dastebandi'");
        }
    }else {
        $D=0;
        $command="WHERE";
        ...
        for ($II=0;$II<count($X);$II=$II+1){
            if (strlen($X[$II])>0){
                if ($D===0){
                    $command=$command." name LIKE '%".$X[$II]."%' OR azmun LIKE '%".$X[$II]."%'";
                    $command2=$command2." name LIKE '%".$X[$II]."%'";
                }else {
                    $command=$command."OR name LIKE '%".$X[$II]."%' OR azmun LIKE '%".$X[$II]."%'";
        ...
        if ($D===0){
            ...
            if ($dastebandi=='all') {
                if ($type == 1)
                    $results = $connection->query("SELECT COUNT(*) FROM karjoo_request " . $command);
            }else{
                if ($type == 1)
                    $results = $connection->query("SELECT COUNT(*) FROM karjoo_request " . $command." and dastebandi='$dastebandi'");
            }
        }
    }
    ...
    if ($query==="=====+++====="){
        ...
        if ($dastebandi=='all') {
            if ($type == 1)
                $results = $connection->query("SELECT * FROM karjoo_request ORDER BY ID DESC  LIMIT $page_position, $item_per_page ");
        }else{
            if ($type == 1)
                $results = $connection->query("SELECT * FROM karjoo_request WHERE dastebandi='$dastebandi' ORDER BY ID DESC  LIMIT $page_position, $item_per_page ");
        }
    }else {
        $command="WHERE";
        $command2="WHERE";
        ...
        if ($D===0){
            ...
            if ($dastebandi=='all') {
                if ($type == 1)
                    $results = $connection->query("SELECT * FROM karjoo_request " . $command . " ORDER BY ID DESC LIMIT $page_position, $item_per_page ");
            }else{
                if ($type == 1)
                    $results = $connection->query("SELECT * FROM karjoo_request " . $command ."and dastebandi='$dastebandi'". " ORDER BY ID DESC LIMIT $page_position, $item_per_page ");
            }
        }
    }
