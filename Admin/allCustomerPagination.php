...
if(isset($_GET) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
    ...
    if ($query==="=====+++=====") {
        if ($dastebandi=='all'){
            if ($type == 1)
                $results = $connection->query("SELECT COUNT(*) FROM customers");
        }else{
            if ($type == 1)
                $results = $connection->query("SELECT COUNT(*) FROM customers WHERE dastebandi='$dastebandi'");
        }
    }else {
        $D=0;
        $command="WHERE";
        ...
        for ($II=0;$II<count($X);$II=$II+1){
            if (strlen($X[$II])>0){
                if ($D===0){
                    $command=$command." title LIKE '%".$X[$II]."%' OR Mokhtasar LIKE '%".$X[$II]."%'";
                    $command2=$command2." title LIKE '%".$X[$II]."%'";
                }else {
                    $command=$command."OR title LIKE '%".$X[$II]."%' OR Mokhtasar LIKE '%".$X[$II]."%'";
                    $command2=$command2."OR title LIKE '%".$X[$II]."%'";
                }
                $D=$D+1;
            }
        }
        if ($D===0){
            ...
            if ($dastebandi=='all') {
                if ($type == 1)
                    $results = $connection->query("SELECT COUNT(*) FROM customers " . $command);
            }else{
                if ($type == 1)
                    $results = $connection->query("SELECT COUNT(*) FROM customers " . $command." and dastebandi='$dastebandi'");
            }
        }
    }
    ...
    if ($query==="=====+++====="){
        ...
        if ($dastebandi=='all') {
            if ($type == 1)
                $results = $connection->query("SELECT * FROM customers ORDER BY ID DESC  LIMIT $page_position, $item_per_page ");
        }else{
            if ($type == 1)
                $results = $connection->query("SELECT * FROM customers WHERE dastebandi='$dastebandi' ORDER BY ID DESC  LIMIT $page_position, $item_per_page ");
        }
    }else {
        $command="WHERE";
        $command2="WHERE";
        ...
        if ($D===0){
            ...
            if ($dastebandi=='all') {
                if ($type == 1)
                    $results = $connection->query("SELECT * FROM customers " . $command . " ORDER BY ID DESC LIMIT $page_position, $item_per_page ");
            }else{
                if ($type == 1)
                    $results = $connection->query("SELECT * FROM customers " . $command ."and dastebandi='$dastebandi'". " ORDER BY ID DESC LIMIT $page_position, $item_per_page ");
            }
        }
    }
...
//echo $row['topic'];
        ...
        echo "                                                                      
                                    <td style=\"width: 20%;\">
                                        ...
                                        <a onClick=\"return confirming();\"  href='deletecustomer.php?type=$type&product=".$row['ID']."' class=\"table-link danger\">
                                            <span class=\"fa-stack\">
                                                <i class=\"fa fa-square fa-stack-2x\"></i>
