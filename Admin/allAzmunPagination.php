...
if(isset($_GET) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
    ...
    if ($query==="=====+++=====") {
        if ($dastebandi=='all'){
            if ($type == 1)
                $results = $connection->query("SELECT COUNT(*) FROM azmun");
        }else{
            if ($type == 1)
                $results = $connection->query("SELECT COUNT(*) FROM azmun WHERE dastebandi='$dastebandi'");
        }
    }else {
        $D=0;
        $command="WHERE";
        ...
        if ($D===0){
            ...
            if ($dastebandi=='all') {
                if ($type == 1)
                    $results = $connection->query("SELECT COUNT(*) FROM azmun " . $command);
            }else{
                if ($type == 1)
                    $results = $connection->query("SELECT COUNT(*) FROM azmun " . $command." and dastebandi='$dastebandi'");
            }
        }
    }
    ...
    if ($query==="=====+++====="){
        ...
        if ($dastebandi=='all') {
            if ($type == 1)
                $results = $connection->query("SELECT * FROM azmun ORDER BY ID DESC  LIMIT $page_position, $item_per_page ");
        }else{
            if ($type == 1)
                $results = $connection->query("SELECT * FROM azmun WHERE dastebandi='$dastebandi' ORDER BY ID DESC  LIMIT $page_position, $item_per_page ");
        }
    }else {
        $command="WHERE";
        $command2="WHERE";
...
//                    $command=$command."OR topic LIKE '%".$X[$II]."%' OR Mokhtasar LIKE '%".$X[$II]."%'";
        ...
        if ($D===0){
            ...
            if ($dastebandi=='all') {
                if ($type == 1)
                    $results = $connection->query("SELECT * FROM azmun " . $command . " ORDER BY ID DESC LIMIT $page_position, $item_per_page ");
            }else{
                if ($type == 1)
                    $results = $connection->query("SELECT * FROM azmun " . $command ."and dastebandi='$dastebandi'". " ORDER BY ID DESC LIMIT $page_position, $item_per_page ");
            }
        }
    }
...
//echo $row['topic'];
        ...
        if ($type==1){
            echo  "                     <td>
                                    ...
                                    <td>
                                        <div class=\"info\">
                                            <a target='_blank' href=\"/azmun/".$row['englishName']." class=\"user-link\">".$row['title']."</a>
                                        </div>
                                    </td>";
            echo "   
                                    <td>                                                                                                               
        ...
        echo "                                                                      
                                    <td style=\"width: 20%;\">
                                        <a  target='_blank' href='addAzmun.php?type=$type&product=".$row['ID']."' class=\"table-link\">
                                            <span class=\"fa-stack\">
                                                <i class=\"fa fa-square fa-stack-2x\"></i>
                                                <i class=\"fa fa-pencil fa-stack-1x fa-inverse bluecolor\"></i>
                                            </span>
                                        </a>
                                        <a onClick=\"return confirming();\"  href='deleteazmun.php?type=$type&product=".$row['ID']."' class=\"table-link danger\">
                                            <span class=\"fa-stack\">
                                                <i class=\"fa fa-square fa-stack-2x\"></i>
