...
//            echo "<script>window.alert('insert db');</script>";
            $stmt  = $connection->prepare("INSERT INTO azmun (xmlAdress,title, dateAzmun, dateKart, dateNatayej,englishName,typ, state)  VALUES (?,?,?,?,?,?,?,?)");
            $stmt->bind_param("ssssssss", $filename,$topic,$dateAzmun,$dateKart,$dateNatayej,$englishtopic,$active,$state);
        } else {
//            echo "<script>window.alert('update db');</script>";
            $stmt  = $connection->prepare("UPDATE azmun SET xmlAdress=?,title=?,dateAzmun=?, dateKart=?,dateNatayej=?,englishName=?typ=?,state=? WHERE ID='$product'");
            $stmt->bind_param("ssssssss", $filename,$topic,$dateAzmun,$dateKart,$dateNatayej,$englishtopic,$active,$state);
        }
